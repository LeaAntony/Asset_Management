...
namespace Asset_Management.Controllers
{
    ...
    public class AuthController : Controller
    {
        ...
        {
            originalPath = originalPath.Replace("[AND]", "&");
            ...
            if (string.IsNullOrWhiteSpace(sesa_id) || string.IsNullOrWhiteSpace(first_name))
            {
            ...
            var manager_sesa_id = User.FindFirst("manager")?.Value;
            var manager_name = User.FindFirst("managerName")?.Value;
            ...
            claimsIdentity.AddClaim(new Claim("asset_management_name", full_name));
            if (userDetail.Any())
            {
                var user = userDetail.First();
                if (!string.IsNullOrEmpty(user.level))
                {
                    claimsIdentity.AddClaim(new Claim("asset_management_level", user.level));
                    claimsIdentity.AddClaim(new Claim("asset_management_role", user.role));
                    claimsIdentity.AddClaim(new Claim("asset_management_role_manage_user", user.role_manage_user.ToString()));
                }
                else
                {
                    claimsIdentity.AddClaim(new Claim("asset_management_level", "no_access"));
            ...
            string redirectUrl = originalPath == "/" ? Url.Action("Open", "Home") : originalPath;
